...
POPULATION_SIZE = 40
OFFSPRING_SIZE = 10
NUM_GENERATIONS = 10*N[GOAL_N]
Individual = namedtuple("Individual", ["genome", "fitness"])
#print(PROBLEM_SPACE)

def fitnessFunc(genome, fit_count = 0):
        fit_count += 1
        tuple_set = set()
        genome_lenght=0
        ...
        return 50*genome_coverage - genome_lenght
...
for genome in [tuple([random.choice([1, 0]) for _ in range(PROBLEM_SIZE)]) for _ in range(POPULATION_SIZE)]:
    population.append(Individual(genome, fitnessFunc(genome, fit_count)))
